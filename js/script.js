...
(function iniciarQuiz() {
  var perguntas = [
    ...
    {
      enunciado: "Qual satélite da NASA/NOAA o GridShield usa para detectar CMEs?",
      opcoes: ["Hubble", "DSCOVR", "ISS", "James Webb"],
      correta: 1
    },
    {
      enunciado: "Em qual ponto Lagrangiano o satélite DSCOVR está posicionado?",
      opcoes: ["L2", "L4", "L1", "L3"],
      correta: 2
    },
    {
    ...
    {
      enunciado: "Qual valor de Kp indica início de perturbação geomagnética (nível G1)?",
      opcoes: ["Kp = 2", "Kp = 9", "Kp = 5", "Kp = 7"],
      correta: 2
    },
    {
      enunciado: "Com quantas horas de antecedência o GridShield pode detectar uma CME?",
      opcoes: ["1 hora", "12 horas", "72 horas", "7 dias"],
      correta: 2
    },
    {
      enunciado: "Qual ODS da ONU o GridShield está alinhado?",
      opcoes: ["ODS 3 — Saúde", "ODS 9 — Inovação e Infraestrutura", "ODS 13 — Ação Climática", "ODS 7 — Energia Limpa"],
      correta: 1
    },
    {
      enunciado: "O que o GridShield faz ao detectar risco iminente de CME severa?",
      opcoes: [
    ...
    {
      enunciado: "Qual modelo matemático descreve o decaimento do Índice Kp após o pico da tempestade?",
      opcoes: [
        "Função linear",
  ...
  function mostrarPergunta(index) {
    ...
    p.opcoes.forEach(function (opcao, i) {
      ...
      btn.addEventListener("click", function () { selecionar(i, btn, p.correta); });
  ...
  function mostrarResultado() {
    ...
    score.textContent = pontos + " de " + perguntas.length + " acertos (" + pct.toFixed(0) + "%)";

    if (pct >= 80) {
      icon.textContent  = "🛰️";
      title.textContent = "Excelente! Você domina o tema!";
      msg.textContent   = "Seu conhecimento sobre clima espacial e proteção de redes elétricas é de alto nível.";
    } else if (pct >= 50) {
      icon.textContent  = "⚡";
      title.textContent = "Bom resultado! Continue aprendendo.";
      msg.textContent   = "Você tem uma boa base sobre o GridShield. Revise os pontos que errou.";
    } else {
      icon.textContent  = "🌍";
      title.textContent = "Ainda há muito para explorar!";
      msg.textContent   = "Leia as seções do site e tente novamente para melhorar seu desempenho.";
    }
  }
...
(function iniciarFormulario() {
  ...
  var campos = {
    nome:  { input: document.getElementById("inputNome"),  erro: document.getElementById("erroNome")  },
    email: { input: document.getElementById("inputEmail"), erro: document.getElementById("erroEmail") },
    org:   { input: document.getElementById("inputOrg"),   erro: document.getElementById("erroOrg")   },
    msg:   { input: document.getElementById("inputMsg"),   erro: document.getElementById("erroMsg")   },
  };
  ...
  Object.keys(campos).forEach(function (k) {
    campos[k].input.addEventListener("input", function () { limparErro(k); });
  });
  ...
  submitBtn.addEventListener("click", function () {
    ...
    Object.keys(campos).forEach(function (k) { limparErro(k); });
